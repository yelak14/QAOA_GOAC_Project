"""QAOA with constraint-preserving XY mixer on IBM Quantum hardware (Free Plan)."""
...
def main():
    print("=" * 60)
    print("QAOA with XY Mixer (IBM Hardware - Free Plan)")
    print("=" * 60)
    ...
    x0 = np.array([0.5, 0.5])  # Better starting point for p=1

    # SPSA optimizer - better for noisy hardware
    optimizer = SPSA(maxiter=60, learning_rate=0.1, perturbation=0.2)
    ...
    print(
        f"Best config: {best_bs}, E = {best_energy:.6f} eV, prob = {best_prob:.3f}")
    ...
    plot_convergence(history['costs'],
                     ...
                     title=f"XY Mixer p={p} (Hardware: {backend.name})")
    ...
    plot_brute_force_comparison(all_energies, counts, alpha, beta_coeff, E_const,
                                ...
                                title=f"XY Mixer (p={p}, {backend.name}) vs Brute-Force")

    # Config vs energy
    plot_config_vs_energy(counts, alpha, beta_coeff, E_const,
                          save_path=str(output_dir / "config_vs_energy.png"),
                          title=f"XY Mixer (p={p}, {backend.name}) - Config vs Energy")
    ...
    plot_approximation_ratio_vs_depth(
        ...
        title=f"XY Mixer ({backend.name}) - Approximation Ratio"
    )
