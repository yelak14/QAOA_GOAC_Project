"""QAOA with standard (X) mixer on AerSimulator."""
...
def main():
    print("=" * 60)
    print("QAOA with Standard Mixer (Simulator)")
    print("=" * 60)
    ...
    # Sort all energies
    sorted_energies = sorted(all_energies.items(), key=lambda x: x[1])
    print(f"\nTop 5 lowest-energy configurations:")
    for bs, e in sorted_energies[:5]:
        print(f"  {bs} -> {e:.6f} eV")
    ...
    for p in [1, 2, 3]:
        ...
        circuit, gammas, betas = build_qaoa_circuit(
            hamiltonian, mixer_type='standard', p=p,
        ...
        if valid_counts:
            ...
            best_bs_corrected = best_bitstring[::-1]  # Reverse for our convention
    ...
    circuit_p1, gammas_p1, betas_p1 = build_qaoa_circuit(
        hamiltonian, mixer_type='standard', p=1,
        n_qubits=n_qubits, n_particles=n_particles
    )
    ...
    # Save results
    output_dir = project_root / "results" / "simulator" / "standard"
    ...
    plot_approximation_ratio_vs_depth(
        ...
        title="Standard Mixer - Approximation Ratio vs Depth"
    )
    ...
    plot_parameter_landscape(
        ...
        title="Standard Mixer (p=1) - Parameter Landscape"
    )

    print("Plots saved to results/simulator/standard/")
    print("\n" + "=" * 60)
    print("DONE")
    print("=" * 60)
